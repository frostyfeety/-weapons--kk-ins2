...
if CLIENT then
    SWEP.PrintName = "Армейская Военная Аптечка"
    SWEP.Slot = 5
    SWEP.SlotPos = 0
...
SWEP.Contact        = ""
SWEP.Purpose        = ""
SWEP.Instructions    = "ЛКМ-бинт,ПКМ-Шина и Гемостоп"
...
function SWEP:Equip(own)
	own:GiveAmmo(10, "Bandages")
	own:GiveAmmo(3, "Quikclots")
	own:GiveAmmo(3, "Hemostats")
end
...
function SWEP:PrimaryAttack()	
	...
	timer.Simple( 2.5, function() self.Owner:ConCommand("gachibandage" ) end )
end
...
function SWEP:SecondaryAttack()	
	...
	if am > 0 then
		...
		timer.Simple( 2.5, function() self.Owner:ConCommand("gachimedkit" ) end )
	else
		am = self.Owner:GetAmmoCount("Quikclots")
